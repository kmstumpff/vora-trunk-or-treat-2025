...
export default class LatchDevice extends BaseDevice {
  ...
  async subscribeToStatusChanges() {
    await this.startStatusNotifications(this.SERVICE_UUID, this.STATUS_CHAR_UUID, (data: string) => {
  ...
  decodeStatus(base64Value: Base64): string {
    ...
    return statusByte === this.UNLOCKED_CMD ? LatchStatus.UNLOCKED : LatchStatus.LOCKED;
  }

  encodeCommand(command: string): Base64 {
    const commandByte = command === LatchStatus.UNLOCKED ? this.UNLOCKED_CMD : this.LOCKED_CMD;
  ...
  async setLatchState(state: LatchStatus): Promise<void> {
    this.writeCommand(this.SERVICE_UUID, this.COMMAND_CHAR_UUID, state.valueOf());
  }
