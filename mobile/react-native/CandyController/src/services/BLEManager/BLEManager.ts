...
export class BLEManager {
  ...
  isServiceCharacteristicMonitorDisconnectExpected = false;

  deviceListeners: Set<(device: BaseDevice | null) => void>;
  ...
  connectToDevice = (deviceId: DeviceId) =>
    new Promise<Device>((resolve, reject) => {
      ...
      this.manager
        ...
        .then((device) => {
          this.device = new BaseDevice(device);
          this.device.initialize();
          this.notifyDeviceListeners(this.device);
          resolve(device);
        })
        .catch((error) => {
  ...
  onDeviceChange = (callback: (device: BaseDevice | null) => void) => {
    this.deviceListeners.add(callback);
    return () => this.deviceListeners.delete(callback);
  };

  notifyDeviceListeners(device: BaseDevice | null) {
